...
def get_coefficients(n):
    a = 2*(-2*n*sin(pi**2)*sin(2*pi*n) - pi*cos(pi**2)
             * cos(2*pi*n) + pi)/(pi**3 - 4*pi*n**2)
    if n == 0:
        return a
    if n > 10:
        return a, 0

    b = (sin(pi*(pi-2*n))/(pi - 2*n) -
           sin(pi*(2*n + pi))/(2*n + pi))/pi
...
def plot_harmonics(n):
    ...
    plt.plot(x, harmonic, label="Гармоніка 0")

    for k in range(1, n + 1):
        a_n, b_n = get_coefficients(k)
        harmonic = [a_n*cos(k*val) + b_n*sin(k*val) for val in x]
        plt.plot(x, harmonic, label=f"Гармоніка {k}")
...
def plot_fourier(n):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(x, y_original, label="sin(πx)")
    plt.plot(x, y_fourier, label=f"Ряд Фур'є порядку {n}", color="orange", lw=1)
    plt.xlabel("x")
    plt.ylabel("f(x)")
    plt.grid(True)
    plt.legend()
