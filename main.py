...
def get_coefficients(N):
    a_N = (1 / pi) * np.trapezoid(
        [f(x) * cos(N * x) for x in np.linspace(0, pi, 1000)],
        np.linspace(0, pi, 1000)
    )
    ...
    b_N = (1 / pi) * np.trapezoid(
        [f(x) * sin(N * x) for x in np.linspace(0, pi, 1000)],
        np.linspace(0, pi, 1000)
    )
...
def relative_error(N):
    ...
    for val in x:
        f_val = f(val)
        f_approx = fourier(val, N)
        if f_val != 0:  # Avoid division by zero
            errors.append(abs(f_val - f_approx) / abs(f_val))
...
def plot_fourier(N):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(x, y_original, label=r"$sin(\pi x)$")
